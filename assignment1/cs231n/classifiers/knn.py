...
class KNearestNeighbor:
    ...
    def train(self, X, y):
        ...
        Args:
            X: numpy массив формой (num_train, D), содержащий обучающие данные, состоящие из
              num_train примеров каждого измерения D.
            y: numpy массив формой (N,), содержащий обучающие метки, где y[i] - метка для X[i].
        """
        self.X_train = X
        self.y_train = y

    def predict(self, X, k=1, num_loops: int = 0):
        """Предсказывает метки для тестовых данных с помощью этого классификатора.

        Args:
            X: numpy массив формой (num_test, D), содержащий тестовые данные, состоящие из num_test
              примеров каждого измерения D.
        ...
        Returns:
            y: numpy массив формой (num_test,), содержащий предсказанные метки для тестовых данных,
              где y[i] - предсказанная метка для тестовой точки X[i].
        """
        if num_loops == 0:
    ...
    def compute_distances_two_loops(self, X):
        """Вычисляет расстояние между каждой тестовой точкой в X и каждой обучающей точкой в
        self.X_train, используя вложенный цикл как по обучающим, так и по тестовым данным.

        Args:
            X: numpy массив формой (num_test, D), содержащий тестовые данные.

        Returns:
            dists: numpy массив формой (num_test, num_train), в котором dists[i, j] - евклидово
              расстояние между i-ой тестовой и j-ой обучающей точкой.
        """
        num_test = X.shape[0]
    ...
    def compute_distances_one_loop(self, X):
        """Вычисляет расстояние между каждой тестовой точкой в X и каждой обучающей точкой в
        self.X_train, используя один цикл по тестовым данным.
    ...
    def predict_labels(self, dists, k=1):
        """Учитывая матрицу расстояний между тестовыми и обучающими точками, предсказывает метку для
        каждой тестовой точки.

        Args:
            dists: numpy массив формой (num_test, num_train), где dists[i, j] - даёт расстояние
              между i-й тестовой и j-й обучающей точкой.

        Returns:
            y_pred: numpy массив формой (num_test,), содержащий предсказанные метки для тестовых
              данных, где y[i] - предсказанная метка для тестовой точки X[i].
        """
